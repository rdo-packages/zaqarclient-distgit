%{!?upstream_version: %global upstream_version %{version}%{?milestone}}

%global sname zaqarclient

%if 0%{?fedora}
%global with_python3 1
%endif

Name:           python-zaqarclient
Version:        XXX
Release:        XXX
Summary:        Client Library for OpenStack Zaqar Queueing API

License:        ASL 2.0
URL:            https://bugs.launchpad.net/python-zaqarclient
Source0:        http://tarballs.openstack.org/python-zaqarclient/%{name}/%{name}-%{version}.tar.gz
BuildArch:      noarch

%description
Python client to Zaqar messaging service API v1

%package -n python2-%{sname}
Summary:        Client Library for OpenStack Zaqar Queueing API
%{?python_provide:%python_provide python2-zaqarclient}

BuildRequires:  python2-devel
BuildRequires:  python-pbr
BuildRequires:  python-setuptools

Requires:       python-jsonschema
Requires:       python-keystoneclient >= 1.6.0
Requires:       python-oslo-i18n >= 1.5.0
Requires:       python-pbr
Requires:       python-requests >= 2.5.2
Requires:       python-six >= 1.9.0
Requires:       python-stevedore >= 1.5.0
Requires:       python-osc-lib

%description -n python2-%{sname}
Python client to Zaqar messaging service API v1

%if 0%{?with_python3}
%package -n python3-%{sname}
Summary:        Client Library for OpenStack Zaqar Queueing API
%{?python_provide:%python_provide python3-zaqarclient}

BuildRequires:  python3-devel
BuildRequires:  python3-pbr
BuildRequires:  python3-setuptools

Requires:       python3-jsonschema
Requires:       python3-keystoneclient >= 1.6.0
Requires:       python3-oslo-i18n >= 1.5.0
Requires:       python3-pbr
Requires:       python3-requests >= 2.5.2
Requires:       python3-six >= 1.9.0
Requires:       python3-stevedore >= 1.5.0
Requires:       python3-osc-lib

%description -n python3-%{sname}
Python client to Zaqar messaging service API v1
%endif

%package doc
Summary:          Documentation for OpenStack Zaqar API Client

BuildRequires:    python-sphinx
BuildRequires:    python-oslo-sphinx

%description      doc
Python client to Zaqar messaging service API v1

%prep
%setup -q -n %{name}-%{upstream_version}

%build
%py2_build
%if 0%{?with_python3}
%py3_build
%endif

%install
%if 0%{?with_python3}
%py3_install
# Delete tests
rm -fr %{buildroot}%{python3_sitelib}/%{sname}/tests
%endif

%py2_install

# Delete tests
rm -fr %{buildroot}%{python2_sitelib}/%{sname}/tests


export PYTHONPATH="$( pwd ):$PYTHONPATH"
sphinx-build -b html doc/source html


%files -n python2-%{sname}
%doc README.rst
%license LICENSE
%{python2_sitelib}/%{sname}
%{python2_sitelib}/*.egg-info

%if 0%{?with_python3}
%files -n python3-%{sname}
%license LICENSE
%doc README.rst
%{python3_sitelib}/%{sname}
%{python3_sitelib}/*.egg-info
%endif

%files doc
%doc html
%license LICENSE

%changelog
